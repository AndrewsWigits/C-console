...
* timeUnitsToString
*
* Converts PS2000_TIME_UNITS enumeration to string (used for streaming mode)
*
****************************************************************************/
...
void BlockDataHandler(UNIT * unit, char * text, int32_t offset, MODE mode)
{
	...
	if (mode == ANALOGUE || mode == MIXED)		// Analogue or  (MSO Only) MIXED 
	{
		for (i = 0; i < unit->channelCount; i++) 
		{
			...
			status = ps2000aSetDataBuffers(unit->handle, (int16_t)i, buffers[i * 2], buffers[i * 2 + 1], sampleCount, 0, PS2000A_RATIO_MODE_NONE);
	...
	*		 the most suitable time units, and the maximum oversample at the current timebase*/
	...
	if(g_ready) 
	{
		status = ps2000aGetValues(unit->handle, 0, (uint32_t*) &sampleCount, 1, PS2000A_RATIO_MODE_NONE, 0, NULL);
...
void StreamDataHandler(UNIT * unit, uint32_t preTrigger, MODE mode)
{
	...
	uint32_t sampleCount = 40000; /*make sure buffer large enough */
	...
	if (mode == ANALOGUE)		// Analogue 
	{
		for (i = 0; i < unit->channelCount; i++) 
		{
			buffers[i * 2] = (int16_t*) malloc(sampleCount * sizeof(int16_t));
			buffers[i * 2 + 1] = (int16_t*) malloc(sampleCount * sizeof(int16_t));
			status = ps2000aSetDataBuffers(unit->handle, (int16_t)i, buffers[i * 2], buffers[i * 2 + 1], sampleCount, 0, PS2000A_RATIO_MODE_AGGREGATE);
	...
	status = ps2000aRunStreaming(unit->handle, &sampleInterval, timeUnits, preTrigger, postTrigger - preTrigger, 
				autostop, downsampleRatio, ratioMode, sampleCount);
